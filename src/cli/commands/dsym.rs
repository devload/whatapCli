...
pub async fn upload(
    ...
    let files: Vec<PathBuf> = if file_path.is_file() {
        ...
        symbol::discover_files(file_path, None, None, SymbolType::Dsym)?
    };

    output::info(
        &format!("Found {} dSYM file(s)", files.len()),
        config.quiet,
    );
...
pub async fn list(config: &ResolvedConfig, pcode: Option<i64>) -> Result<()> {
    ...
    let files = symbol::list_files(&client, pcode, SymbolType::Dsym).await?;
    ...
    output::info(
        &format!("Found {} dSYM file(s) for pcode {}", rows.len(), pcode),
        config.quiet,
    );
...
pub async fn delete(
    ...
    if !confirm {
        ...
        output::warn(&format!(
            "This will delete dSYM file(s) matching '{}' for pcode {}. Use --confirm to proceed.",
            target, pcode
        ));
        return Ok(());
    }

    symbol::delete_files(&client, pcode, SymbolType::Dsym, file, version).await?;
    output::success("dSYM file(s) deleted");
